...
set(COBRA_TOOLS_OUTPUT_DIR "${CMAKE_BINARY_DIR}/bin/${CMAKE_CFG_INTDIR}")
...
function(cobra_update_compile_flags name)
...
function(add_cobra_library name)
  cmake_parse_arguments(ARG "" "" "LINK_LIBS" ${ARGN})
  add_library(${name} STATIC ${ARG_UNPARSED_ARGUMENTS})
  target_link_libraries(${name} ${ARG_LINK_LIBS} ${COBRA_LINK_COMPONENTS})
  set_property(TARGET ${name} PROPERTY POSITION_INDEPENDENT_CODE ON)
  cobra_update_compile_flags(${name})
endfunction(add_cobra_library)

function(add_cobra_executable name)
  cmake_parse_arguments(ARG "" "" "LINK_LIBS" ${ARGN})
  add_executable(${name} ${ARG_UNPARSED_ARGUMENTS})
  target_link_libraries(${name} ${ARG_LINK_LIBS} ${COBRA_LINK_COMPONENTS})
  target_link_options(${name} PRIVATE ${COBRA_EXTRA_LINKER_FLAGS})
  cobra_update_compile_flags(${name})
endfunction(add_cobra_executable)

function(add_cobra_tool name)
  add_cobra_executable(${name} ${ARGN})

  # In multi-config build systems, remove the per-config directory and let
  # CMake automatically add it in the component.
  if (NOT "${CMAKE_CFG_INTDIR}" STREQUAL ".")
    string(REPLACE ${CMAKE_CFG_INTDIR} "" COBRA_TOOLS_OUTPUT_DIR "${COBRA_TOOLS_OUTPUT_DIR}")
  endif ()

  set_property(TARGET ${name} PROPERTY RUNTIME_OUTPUT_DIRECTORY
    "${COBRA_TOOLS_OUTPUT_DIR}")
endfunction(add_cobra_tool)
